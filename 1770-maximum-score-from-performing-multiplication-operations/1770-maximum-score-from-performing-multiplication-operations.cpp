...
public:
    ...
    int helper(int index,vector<int>& nums, vector<int>& multi, int start){ 
        ...
        int left = (multi[index]*nums[start]) + helper(index+1, nums,multi,start+1);
        
        int right = (multi[index]*nums[end]) + helper(index+1, nums, multi, start);
        
        return dp[index][start]= max(left,right);
        }
